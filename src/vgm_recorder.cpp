...
void VgmRecorder::Stop()
{
    ...
    if (file.is_open())
    {
        ...
        // Version number (1.72 = 0x00000172)
        header[0x08] = 0x72;
        header[0x09] = 0x01;
        header[0x0A] = 0x00;
        header[0x0B] = 0x00;
...
void VgmRecorder::WriteMikey(u16 address, u8 data)
{
    if (!m_recording)
        return;
    ...
    // 0x40 aa dd - Mikey, write value dd to register aa
    // Valid range: 0xFD00-0xFD50 (Mikey registers)
    // Convert to VGM register format (0x00-based)
    if (address >= 0xFD00 && address <= 0xFD50)
    {
        u8 reg = address - 0xFD00;
        WriteCommand(0x40, reg, data);
    }
    else
    {
