...
@interface GearlynxApplicationDelegate : NSObject <NSApplicationDelegate>
{
    id m_delegate;
    NSMenu* m_dock_menu;
}
...
@implementation GearlynxApplicationDelegate
...
- (void)launchNewInstance:(id)sender
{
    ...
    {
        if (error)
            NSLog(@"Failed to launch new Gearlynx window: %@", error);
    }];
}
...
static GearlynxApplicationDelegate* gearlynx_application_delegate = nil;
...
{
    if (gearlynx_application_delegate || ![[[[NSBundle mainBundle] bundleURL] pathExtension] isEqualToString:@"app"])
        return;
    ...
    gearlynx_application_delegate = [[GearlynxApplicationDelegate alloc] initWithDelegate:delegate];
    [NSApp setDelegate:gearlynx_application_delegate];
}

extern "C" void macos_remove_dock_menu(void)
{
    if (!gearlynx_application_delegate)
        return;

    if ([NSApp delegate] == gearlynx_application_delegate)
        [NSApp setDelegate:[gearlynx_application_delegate originalDelegate]];

    [gearlynx_application_delegate release];
    gearlynx_application_delegate = nil;
}

extern "C" void macos_launch_new_instance(void)
{
    if (gearlynx_application_delegate)
        [gearlynx_application_delegate launchNewInstance:nil];
}
