...
final class EmulationViewController: UIViewController, UIAdaptivePresentationControllerDelegate {
    private var rom: Rom
    private let emulator = GearlynxEmulator()
    ...
    private var screenAspectRatio: CGFloat = 4.0 / 3.0
    private var nativeScreenWidth: CGFloat = 160.0
    private var nativeScreenHeight: CGFloat = 102.0
    ...
    override func viewDidLayoutSubviews() {
        ...
        layoutScreen()
        if controlsView.positionPortraitControls(after: screenView.frame.maxY) {
            controlsView.layoutIfNeeded()
            layoutScreen()
        }
    }
    ...
    private func configureViews() {
        ...
        emulator.configure(
            console: AppSettings.console.rawValue,
            eeprom: AppSettings.eeprom.rawValue,
            cartridgeHardware: AppSettings.cartridgeHardware.rawValue,
            legacySpriteRenderer: AppSettings.legacySpriteRendererEnabled,
            rotation: AppSettings.rotation.rawValue,
            lowpassCutoff: AppSettings.lowpassCutoff.rawValue,
            saveStateSlot: AppSettings.saveStateSlot,
            firmwareDirectory: PathUtils.getFirmwareDir
        )
    ...
    private func layoutScreen() {
        ...
        if landscape {
            ...
            let right = controlsView.option1.frame.minX - 12.0
    ...
    private func emulatorButton(for button: GameControllerInput.Button) -> GearlynxButton? {
        switch button {
        case .up: return .up
        ...
        case .a: return .A
        case .b: return .B
        case .x: return .option1
        case .y: return .option2
