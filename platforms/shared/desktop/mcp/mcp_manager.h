...
public:
    ...
    void PumpCommands(GearlynxCore* core)
    {
        u64 current_cycles = core->GetTotalCycles();

        for (size_t i = 0; i < m_delayedReleases.size(); )
        {
            if (current_cycles >= m_delayedReleases[i].release_at_cycle)
            {
                m_debugAdapter->ControllerButton(m_delayedReleases[i].button, "release");
...
private:
    ...
    json start_input_macro(const json& arguments, const json& request_id)
    {
        ...
        {
            std::string error;
            if (!append_input_macro_command(commands[i], error))
            {
                reset_input_macro();
    ...
    bool append_input_macro_command(const json& command, std::string& error)
    {
        ...
        {
            if (it.key() != "tap" && it.key() != "press" && it.key() != "release" && it.key() != "wait")
            {
        ...
        {
            append_button_step(MCP_INPUT_MACRO_STEP_PRESS, button);
            append_wait_step(1);
            append_button_step(MCP_INPUT_MACRO_STEP_RELEASE, button);
        }
    ...
    bool is_valid_button_name(const std::string& button) const
    {
        ...
        return button_lower == "up" || button_lower == "down" ||
            ...
            button_lower == "option1" || button_lower == "option2" ||
            button_lower == "pause";
    }

    void append_button_step(McpInputMacroStepType type, const std::string& button)
    {
    ...
    void pump_input_macro(GearlynxCore* core)
    {
        ...
        while (m_inputMacro.step_index < m_inputMacro.steps.size())
        {
            ...
            json action_result = m_debugAdapter->ControllerButton(step.button, action);
    ...
    void handle_controller_side_effects(GearlynxCore* core, json& result)
    {
        if (result.contains("__delayed_release") && result["__delayed_release"] == true)
        {
            ...
            release.release_at_cycle = core->GetTotalCycles() + 2500000; // around 10 frames
